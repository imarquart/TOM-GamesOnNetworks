...
Fundamentally, networks transmit the actions of agents over their ties. If our reaction functions are linear, then this is broadly a multiplication of the vector of actions with the adjacency matrix. Think of a dynamic system, where each actor reacts to the past choices of his or her peers (note how equilibria and steady states of dynamic models are similar? That's quite a general outcome!) An equilibrium must now be such that this multiplicative behavior reaches a steady state - it must be (in some way) proportional to an eigenvector. If that is so, the eigenvalues will (in some way) define these equilibria for this particular network!
